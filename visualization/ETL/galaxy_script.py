...
print("Mission initialised, Galaxy schema ETL underway..!")
...
# In Galaxy (Fact Constellation) schema, multiple fact tables share dimension tables.
# ---------------------------------------------------------------------------
...
# Create Fact Table 2: Shipping (shares Time, Product, Location with Sales fact)
cursor.execute("""
...
for i, (_, row) in enumerate(df.iterrows(), start=1):
    ...
    # Map attributes to foreign keys
...
# Load Shipping Fact Table
print("Loading fact_shipping...")
for i, (_, row) in enumerate(df.iterrows(), start=1):
    # Determine ship date, falling back to order date if missing
    s_date = row['ship_date'] if pd.notna(row['ship_date']) else row['order_date']
    if pd.isna(s_date):
        print_progress("fact_shipping", i, len(df))
        continue
    
    # Map attributes to shared and specific foreign keys
