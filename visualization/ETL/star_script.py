...
def get_connection(retries=5, delay=5):
    ...
    Args:
        retries (int): Number of attempts to connect.
...
def execute_batch_safe(conn, cursor, sql, data, batch_size=500, progress_label=None,
    ...
    """
    Inserts data into the database in batches, ensuring data is committed frequently.
    It includes automatic reconnection logic if the database connection drops.
    
    Args:
        conn: The database connection.
        cursor: The database cursor.
        sql (str): The SQL INSERT statement.
        data (list): List of tuples containing the data to insert.
        batch_size (int): Number of rows to insert per batch.
        progress_label (str): Label for the progress bar.
        progress_file (str): Path to save progress for resuming later.
        base_index (int): Starting index for progress tracking.
    """
    total = len(data)
...
def save_progress(phase, row=0):
    """Saves the current phase and row index to the progress file."""
    with open(PROGRESS_FILE, "w") as f:
        f.write(f"{phase}|{row}")
...
# ---------------------------------------------------------------------------
if phase in ("start",):
    # Load Time Dimension: Unique dates from both order and ship dates
    ...
    # Load Customer Dimension: Unique customer names and segments
    ...
    # Load Product Dimension: Unique product IDs with metadata
    ...
    # Load Location Dimension: Unique geographic attributes
    ...
    # Load Shipping Dimension: Unique shipping modes and priorities
...
# ---------------------------------------------------------------------------
# Build lookup maps for fast ID retrieval
# ---------------------------------------------------------------------------
print("Building lookup maps...")
# Fetch existing IDs from dimension tables to map them to CSV values
...
if phase in ("dims_done",):
    ...
    # Iterate through CSV rows to construct the fact table records
    for i, (_, row) in enumerate(df.iterrows(), start=1):
        ...
        # Use lookup maps to find the corresponding foreign key IDs
        ...
        # Skip rows if any required dimension ID is missing
        if not all([date_id, customer_id, location_id, ship_mode_id]):
            print_progress("fact_table", i, total)
            continue

        # Prepare the row for the fact table
        batch.append((
        ...
        if len(batch) >= BATCH_SIZE:
            ...
            # Save progress after each successful batch commit
...
# Remove progress file if successfully completed
if os.path.exists(PROGRESS_FILE):
