...
def execute_batch_safe(conn, cursor, sql, data, batch_size=500,
                       progress_label=None, schema="snowflake"):
    """Insert rows in batches with reconnect on failure."""
    total = len(data)
...
# ---------------------------------------------------------------------------
# Build lookup maps (always needed for fact loading)
# ---------------------------------------------------------------------------
print("Building lookup maps...")
cursor.execute("SELECT date, date_id FROM time_dim")
date_map = dict(cursor.fetchall())

cursor.execute("SELECT customer_name, segment_id, customer_id FROM customer_dim")
